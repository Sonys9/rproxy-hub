...
fn display_banner(listen_ip: SocketAddr, forward_to: SocketAddr, proxies_path: PathBuf) {
    ...
    // 5.2. %color_fg_reset% - Foreground (text) color reset marker (it's a color marker from COLORS in colors.rs)
    // 5.3. %color_fg_NAME% - Foreground (text) color marker where NAME is string color name (example: red, you can get available in COLORS from colors.rs)
    // 5.4. %color_bg_rgb_R_G_B% - Same but for background
    // 5.5. %color_bg_NAME% - Same but for background (also for reset)
